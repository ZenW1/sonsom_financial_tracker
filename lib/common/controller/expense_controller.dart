...
class ExpenseController extends GetxController with StateMixin<List<ExpenseModel>>, BaseController {
  ...
  Future<void> addExpenseRecord({required ExpenseModel data}) async {
    change(null, status: RxStatus.loading());
    try {
      bool result = await _expenseRepository.addExpenseData(data: data);
  ...
  Future<void> deleteExpenseRecord({required String id}) async {
    change(null,status: RxStatus.loading());
    try {
      bool result = await _expenseRepository.deleteExpenseData(id: id);
  ...
  Future<void> getExpenseData() async {
    ...
    try {
      final data = await _expenseRepository.getExpenseData();
